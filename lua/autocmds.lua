...
M.load = function(opts)
  ...
  for event, sound in pairs(sounds) do
    vim.api.nvim_create_autocmd(event, {
      ...
      pattern = "*",
      callback = function()
        if (sound.match == vim.fn.expand("<amatch>")) then
          ...
          vim.api.nvim_create_autocmd("User", {
            ...
            pattern = "*",
            callback = function()
              if("killreverb" == vim.fn.expand("<amatch>")) then
                local len = 0
                for _ in pairs(utils.Pids) do len = len + 1 end
                for pid = 1, len do
                  vim.uv.kill(table.remove(utils.Pids), nil)
                end  end
